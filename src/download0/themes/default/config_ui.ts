...
(function () {
  ...
  // مصفوفة اللغات المدعومة لتسهيل التبديل بينها
  const availableLocales = ['en', 'ar', 'fr', 'es', 'pt', 'de', 'it', 'nl', 'pl', 'tr', 'ja', 'ko', 'zh']
  ...
  if (useImageText) {
    ...
    title.text = lang.config; title.x = 910; title.y = 120; title.style = 'title'
    jsmaf.root.children.push(title)
  }
  ...
  const configOptions = [
    ...
    { key: 'language', label: lang.language || 'Language', imgKey: 'language', type: 'cycle' } 
  ]
  ...
  for (let i = 0; i < configOptions.length; i++) {
    ...
    buttons.push(button); jsmaf.root.children.push(button)
    ...
    if (useImageText) {
      btnText = new Image({
        ...
        x: btnX + 20, y: btnY + 10, width: 180, height: 50
      })
    } else {
      btnText = new jsmaf.Text()
      btnText.text = configOption.label; btnText.x = btnX + 30; btnText.y = btnY + 25; btnText.style = 'white'
    }
    ...
    if (configOption.type === 'toggle') {
      const checkmark = new Image({
        ...
        x: btnX + 320, y: btnY + 15, width: 40, height: 40
      })
      valueTexts.push(checkmark); jsmaf.root.children.push(checkmark)
    } else {
      ...
      if (configOption.key === 'jb_behavior') {
        if (useImageText) {
          valueLabel = new Image({
            ...
            x: btnX + 230, y: btnY + 10, width: 150, height: 50
          })
        } else {
          valueLabel = new jsmaf.Text()
          valueLabel.text = jbBehaviorLabels[currentConfig.jb_behavior] || jbBehaviorLabels[0]!
          valueLabel.x = btnX + 250; valueLabel.y = btnY + 25; valueLabel.style = 'white'
        }
      } else if (configOption.key === 'theme') {
        const themeIndex = availableThemes.indexOf(currentConfig.theme)
        valueLabel = new jsmaf.Text()
        valueLabel.text = themeLabels[themeIndex >= 0 ? themeIndex : 0]!
        valueLabel.x = btnX + 250; valueLabel.y = btnY + 25; valueLabel.style = 'white'
      } else if (configOption.key === 'language') { 
        valueLabel = new jsmaf.Text()
        valueLabel.text = currentConfig.language.toUpperCase()
        valueLabel.x = btnX + 270; valueLabel.y = btnY + 25; valueLabel.style = 'white'
      }
      valueTexts.push(valueLabel!); jsmaf.root.children.push(valueLabel!)
    }
    buttonOrigPos.push({ x: btnX, y: btnY }); textOrigPos.push({ x: btnText.x, y: btnText.y })
  }
  ...
  function animateZoomIn (btn: Image, text: any, bOX: number, bOY: number, tOX: number, tOY: number) {
    ...
    zoomInInterval = jsmaf.setInterval(function () {
      elapsed += 16; const t = Math.min(elapsed / 175, 1), eased = easeInOut(t), scale = 1.0 + (0.1 * eased)
      btn.scaleX = scale; btn.scaleY = scale; btn.x = bOX - (400 * (scale - 1)) / 2; btn.y = bOY - (70 * (scale - 1)) / 2
      text.scaleX = scale; text.scaleY = scale; text.x = tOX - (400 * (scale - 1)) / 2; text.y = tOY - (70 * (scale - 1)) / 2
  ...
  function animateZoomOut (btn: Image, text: any, bOX: number, bOY: number, tOX: number, tOY: number) {
    ...
    zoomOutInterval = jsmaf.setInterval(function () {
      elapsed += 16; const t = Math.min(elapsed / 175, 1), eased = easeInOut(t), scale = 1.1 - (0.1 * eased)
      btn.scaleX = scale; btn.scaleY = scale; btn.x = bOX - (400 * (scale - 1)) / 2; btn.y = bOY - (70 * (scale - 1)) / 2
      text.scaleX = scale; text.scaleY = scale; text.x = tOX - (400 * (scale - 1)) / 2; text.y = tOY - (70 * (scale - 1)) / 2
  ...
  function updateHighlight () {
    if (prevButton >= 0 && prevButton !== currentButton) {
      buttons[prevButton]!.url = normalButtonImg; buttons[prevButton]!.alpha = 0.7
    ...
    buttons[currentButton]!.url = selectedButtonImg; buttons[currentButton]!.alpha = 1.0; buttons[currentButton]!.borderWidth = 3
  ...
  function updateValueText (index: number) {
    const options = configOptions[index]; const valueText = valueTexts[index]
    ...
    if (options.type === 'toggle') {
      ...
      } else if (key === 'theme') (valueText as jsmaf.Text).text = currentConfig.theme
      else if (key === 'language') (valueText as jsmaf.Text).text = currentConfig.language.toUpperCase()
    }
  }
  ...
  function saveConfig () {
    ...
    fs.write('config.json', JSON.stringify(configData, null, 2), function (err) { if (!err) log('Config saved') })
  }
  ...
  function handleButtonPress () {
    ...
    if (option.type === 'cycle') {
      if (key === 'jb_behavior') currentConfig.jb_behavior = (currentConfig.jb_behavior + 1) % jbBehaviorLabels.length
        ...
        // التبديل بين جميع اللغات المتاحة في المصفوفة
        const idx = availableLocales.indexOf(currentConfig.language)
        currentConfig.language = availableLocales[(idx + 1) % availableLocales.length]!
      }
    } else {
  ...
  jsmaf.onKeyDown = function (code) {
    if (code === 6 || code === 5) currentButton = (currentButton + 1) % buttons.length
       ...
       jsmaf.setTimeout(function () { debugging.restart() }, 200)
    }
    updateHighlight()
  }
})()